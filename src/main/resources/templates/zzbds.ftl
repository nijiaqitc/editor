...
<body>
	...
	<div style="float:right;width:18%;height:100%;margin-left: 10px;" ondblclick="showgz()" title="双击展示正则规则">
	...
	<div style="float:right;width:58%;height:100%;">
		<div class="panel panel-default" style="margin-bottom: 0px;">
		  <div class="panel-body" >
		    <textarea style="width:100%;height:20%;border: 0;outline:none;border-bottom: 1px solid #ddd;"  placeholder="输入待加密内容"  id="area1">测a试b文c本</textarea>
		  	<div style="height: 34px;margin: 10px 10px;min-width: 670px;">
			  	<div style="float:left;">
				    <button type="button" class="btn btn-danger" onclick="exec()">exec</button>
				    <button type="button" class="btn btn-danger" onclick="match()">match</button>
				    <button type="button" class="btn btn-danger" onclick="test()">test</button>
			  	</div>
			  	<div style="float:left;width: 460px;margin-left:20px;">
				    <div class="input-group" style="width: 100%;">
					  <input type="text" class="form-control" placeholder="请输入正则表达式"  aria-describedby="basic-addon2" id="zzbds" value="/[\u4e00-\u9fa5]/g">
					</div>
			    </div>
		  	</div>
	...
	<div align="center" class="zzys" ondblclick="showgz()" >
		<div style="clear: both;width: 1000px;" id="gzb" >
			<table class="zztable">
				<thead>
					<tr>
	...
	<script type="text/javascript">
		function exec() {
			...
			var exval = eval($("#zzbds").val()).exec($("#area1").val());
			if (exval) {
				$("#area2").val(exval);
			} else {
				$("#area2").val("无匹配结果");
		...
		function match() {
			...
			var exval = $("#area1").val().match(eval($("#zzbds").val()));
			if (exval) {
		...
		function test() {
			...
			var exval = eval($("#zzbds").val()).test($("#area1").val());
			$("#area2").val(exval);
		}
		...
		function showgz(){
			if(!showFlag){
				$(".zzys").stop().animate({marginTop: 0}, 80);			
				showFlag = true;
			}else{
				$(".zzys").stop().animate({marginTop: -2000}, 80);			
				showFlag = false;
			}
